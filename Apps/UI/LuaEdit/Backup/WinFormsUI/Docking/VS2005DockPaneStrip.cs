...
namespace WeifenLuo.WinFormsUI.Docking
{
	internal class VS2005DockPaneStrip : DockPaneStripBase
	{
        ...
        private const int _DocumentStatusIconGapLeft = 2;
        private const int _DocumentStatusIconGapRight = 3;
        ...
        private static int DocumentStatusIconGapLeft
        {
            get { return _DocumentStatusIconGapLeft; }
        }

        private static int DocumentStatusIconGapRight
        {
            get { return _DocumentStatusIconGapRight; }
        }
		...
		private int GetMaxTabWidth_Document(int index)
		{
			...
			if (DockPane.DockPanel.ShowDocumentIcon)
				overallSize += sizeText.Width + DocumentIconWidth + DocumentIconGapLeft + DocumentIconGapRight + DocumentTextGapRight;
			else
				overallSize += sizeText.Width + DocumentIconGapLeft + DocumentTextGapRight;

            if (content.DockHandler.ShowStatusIcon && content.DockHandler.StatusIcon != null)
                overallSize += content.DockHandler.StatusIcon.Width + DocumentStatusIconGapLeft + DocumentStatusIconGapRight;
		...
		private void DrawTab_Document(Graphics g, TabVS2005 tab, Rectangle rect)
		{
            ...
            Rectangle rectStatusIcon = Rectangle.Empty;
            if (tab.Content.DockHandler.ShowStatusIcon && tab.Content.DockHandler.StatusIcon != null)
            {
                rectStatusIcon = new Rectangle(
                    rectText.X + rectText.Width + DocumentStatusIconGapLeft - tab.Content.DockHandler.StatusIcon.Width - DocumentStatusIconGapRight,
                    rect.Y + rect.Height - 1 - DocumentIconGapBottom - tab.Content.DockHandler.StatusIcon.Height -
                    ((rect.Height - 1 - DocumentIconGapBottom - tab.Content.DockHandler.StatusIcon.Height) / 2),
                    tab.Content.DockHandler.StatusIcon.Width,
                    tab.Content.DockHandler.StatusIcon.Height);
                //tab.TabWidth += rectStatusIcon.Width + DocumentStatusIconGapRight;
            }

            Rectangle rectTab = DrawHelper.RtlTransform(this, rect);
            rectText = DrawHelper.RtlTransform(this, rectText);
            rectIcon = DrawHelper.RtlTransform(this, rectIcon);
            if (rectStatusIcon != Rectangle.Empty)
                rectStatusIcon = DrawHelper.RtlTransform(this, rectStatusIcon);
            ...
            if (tab.Content.DockHandler.ShowStatusIcon && tab.Content.DockHandler.StatusIcon != null && rectStatusIcon != Rectangle.Empty)
                g.DrawIcon(tab.Content.DockHandler.StatusIcon, rectStatusIcon);
