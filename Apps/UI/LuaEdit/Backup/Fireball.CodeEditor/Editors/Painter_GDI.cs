...
namespace Fireball.Windows.Forms.CodeEditor.Painter
{
    ...
    public class Painter_GDI : IPainter
    {
        ...
        private void SetBrackets()
        {
            ...
            Word CurrLeftWord = Control.Caret.CurrentLeftWord;
            Word CurrRightWord = Control.Caret.CurrentRightWord;
            if (CurrLeftWord != null || CurrRightWord != null)
            {
                if (CurrLeftWord != null)
                {
                    if (CurrLeftWord.Pattern != null &&
                        CurrLeftWord.Pattern.BracketType == BracketType.EndBracket)
                    {
                        Word w = this.Control.Document.GetStartBracketWord(CurrLeftWord,
                                                                           CurrLeftWord.Pattern.MatchingBracket, CurrLeftWord.Segment);
                        this.BracketEnd = CurrLeftWord;
                        this.BracketStart = w;
                    }
                }

                if (CurrRightWord != null)
                {
                    if (CurrRightWord.Pattern != null &&
                        CurrRightWord.Pattern.BracketType == BracketType.StartBracket)
                    {
                        Word w = this.Control.Document.GetEndBracketWord(CurrRightWord,
                                                                         CurrRightWord.Pattern.MatchingBracket, CurrRightWord.Segment);
                        this.BracketEnd = w;
                        this.BracketStart = CurrRightWord;
                    }
                }
            }
        }
            ...
            if (RowIndex <= Control.Document.Count - 1)
            {
                ...
                for (int i = 0; i < wordCollection.Count;i++)
                {
                    ...
                    if (StartDraw)
                    {
                        ...
                        {
                            // Draw line breakpoint highlights
                            if (w.Type == WordType.xtWord)
                                DrawBreakpoint = true;

                            if (xtr.Breakpoint != null && DrawBreakpoint)
                            {
        ...
        private void RenderMargin(int RowIndex)
        {
            ...
            if (Control.ShowGutterMargin)
            {
                ...
                if (RowIndex <= Control.Document.Count - 1)
                {
                    ...
                    {
                        if (r.Bookmark != null)
                            Graphics.FromHdc(bbuff.hDC).DrawImage(r.Bookmark.Enabled ? r.Bookmark.BookmarkEnabledImage : r.Bookmark.BookmarkDisabledImage, 0, 0);
                        if (r.Breakpoint != null)
                            Graphics.FromHdc(bbuff.hDC).DrawImage(r.Breakpoint.Enabled ? r.Breakpoint.BreakpointEnabledImage : r.Breakpoint.BreakpointDisabledImage, 0, 0);
                    ...
                    else
                    {
                        ...
                        if (r.Bookmark != null)
                            Graphics.FromHdc(bbuff.hDC).DrawImage(r.Bookmark.Enabled ? r.Bookmark.BookmarkEnabledImage : r.Bookmark.BookmarkDisabledImage, 0, 0, w, w);
                        if (r.Breakpoint != null)
                            Graphics.FromHdc(bbuff.hDC).DrawImage(r.Breakpoint.Enabled ? r.Breakpoint.BreakpointEnabledImage : r.Breakpoint.BreakpointDisabledImage, 0, 0, w, w);
